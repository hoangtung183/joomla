...
// Analyze Skype URI Button
function analyzeSkypeUri(analyzeType, crumbIndex) {

    var _t = Skype.analyzeCrumbs[crumbIndex];

    if (analyzeType === "init") {
        if (_t.prop0 === this.chatLinks.name) {
            _t.prop11 = "Chat Init";
        }
        else if (_t.prop0 === this.callLinks.name) {
            _t.prop11 = "Call Init";
        }
        else if (_t.prop0 === this.dropdownLinks.name) {
...
// Catch any URI that happened before this script finish loading
while (Skype.analyzePreCrumbs.length > 0) {
    var analyzeObj = Skype.analyzePreCrumbs.pop();
    s.trackAction(analyzeObj, this);
}
